...
int main(void)
{
    ...
    typedef thrust::transform_iterator<clamp<int>, thrust::transform_iterator<thrust::negate<int>, CountingIterator>> ClampedCountingIterator;

    CountingIterator count_begin(-9);
    CountingIterator count_end(0);
    
    print_range("sequence         ", count_begin, count_end);

    ClampedCountingIterator cs_begin = thrust::make_transform_iterator(thrust::make_transform_iterator(count_begin, thrust::negate<int>()), clamp<int>(lo, hi));
    ClampedCountingIterator cs_end   = thrust::make_transform_iterator(thrust::make_transform_iterator(count_end, thrust::negate<int>()), clamp<int>(lo, hi));

    print_range("negated and then clamped sequence ", cs_begin, cs_end);
